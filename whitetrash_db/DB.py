...
syslog.openlog('Whitetrash DB Handler',0,syslog.LOG_DAEMON)
...
class PyFlagCursor(MySQLdb.cursors.SSDictCursor):
    ...
    def execute(self,string):
        ...
        def cancel():
            syslog.syslog(syslog.LOG_WARNING, "Killing query in thread %s because it took too long" % self.connection.thread_id())
        ...
        if self._warnings:
            ...
            if len(results)<1000:
                ...
                while 1:
                    a=self.fetchone()
                    if not a: break
                    syslog.syslog(syslog.LOG_DEBUG,"Mysql warnings: query %r: %s" % (last_executed[:500],a))
                else:
                    syslog.syslog(syslog.LOG_DEBUG,"Mysql issued warnings but we are unable to drain result queue")
...
class Pool(Queue):
    ...
    def connect(self):
        ...
        db_connections +=1
        if STRICTSQL:
            dbh = MySQLdb.Connect(user = DBUSER,
                                    passwd = DBPASSWD,
                                    db = case,
        ...
        mysql_bin_string = "%s -f -u %r -p%r -S%s" % (MYSQL_BIN,DBUSER,DBPASSWD,DBUNIXSOCKET)
    ...
    def execute(self,query_str,params=None):
        ...
        #If anything went wrong we raise it as a DBError
            ...
            if 'Commands out of sync' in str or 'server has gone away' in str or 'Lost connection' in str:
                ...
                ## to the DB
                syslog.syslog(syslog.LOG_DEBUG, "Killing connection because %s. Last query was %s" % (e,self.cursor._last_executed))
...
##                key = "%s/%s" % (self.case, threading.currentThread().getName())
    ...
    def check_index(self, table, key, length=None):
        ...
        if key not in fields:
            ...
            syslog.syslog(syslog.LOG_DEBUG,"Oops... No index found in table %s on field %s - Generating index, this may take a while" %(table,key))
...
#            except: pass
    ...
    def MySQLHarness(self,client):
        ...
        if not client.startswith('/'):
            client = "%s/%s" % (FLAG_BIN, client)
            
        syslog.syslog(syslog.LOG_DEBUG, "Will shell out to run %s " % client)
